...
# 标注文件所在的文件夹路径
annotations_dir = '/mnt/d/CV_Code/YOLOv8/ultralytics/ultralytics/datasets/LS-SSDD/biaozhu'
        ...
        with open(os.path.join(annotations_dir, filename), 'r') as file:
            for line in file:
                ...
                if len(parts) == 5:  # 确保有足够的元素
                    ...
                    annotations.append({'class': class_name, 'x_center': x_center, 'y_center': y_center, 'width': width,
                                        'height': height})
                else:
...
plt.ylabel('height', fontsize=14)  # 设置y轴标签字号为14

# 设置x轴和y轴的范围和刻度间隔
plt.xlim(-0.05, 1.05)  # 设置x轴范围为-0.05到1.05
plt.ylim(-0.05, 0.85)  # 设置y轴范围为-0.05到0.85
plt.xticks(ticks=np.arange(0, 1.1, 0.2), fontsize=12)  # x轴刻度间隔为0.2，字号为12
plt.yticks(ticks=np.arange(0, 0.81, 0.1), fontsize=12)  # y轴刻度间隔为0.1，字号为12
