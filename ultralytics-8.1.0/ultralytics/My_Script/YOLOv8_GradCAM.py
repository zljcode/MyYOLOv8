'''为了确保每次运行程序时生成的结果不会覆盖上一次的结果，可以在保存结果时为每次运行生成一个唯一的目录。可以通过添加一个时间戳或一个递增的计数器来实现这一点。以下是修改后的代码：'''
"""用于热力图的生成"""
import warnings
...
class yolov8_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, show_box, renormalize):
        ...
        method = eval(method)(model=model, target_layers=target_layers)
    ...
    def __call__(self, img_path, save_path):
        ...
        unique_save_path = os.path.join(save_path, timestamp)
        ...
        if os.path.isdir(img_path):
            for img_path_ in os.listdir(img_path):
                self.process(f'{img_path}/{img_path_}', f'{unique_save_path}/{img_path_}')
        else:
            self.process(img_path, f'{unique_save_path}/result.png')


def get_params():
    params = {
        'weight': '/mnt/d/CV_Code/YOLOv8/ultralytics/ultralytics/runs/train/YOLOv8_HRSID/weights/best.pt',
...
if __name__ == '__main__':
    ...
    # model(r'/home/hjj/Desktop/dataset/dataset_visdrone/VisDrone2019-DET-test-dev/images/9999947_00000_d_0000026.jpg',
    # 'result')
    model(r'/mnt/d/CV_Code/YOLOv8/ultralytics/ultralytics/datasets/HRSID_JPG/images/test',
          '/mnt/d/CV_Code/YOLOv8/ultralytics/ultralytics/cam_results')
